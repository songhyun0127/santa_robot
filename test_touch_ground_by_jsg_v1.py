...
def main(args=None):
    ...
    def force_based_movement(position_obj, pose_type, VELOCITY, ACC, max_force=10):
        """
        ...
        while not check_force_condition(DR_AXIS_Z, max=max_force):
            if pose_type == 'posx' :
                movel(position_obj, vel=VELOCITY, acc=ACC)
                wait(0.5)
                # check_force_condition = 10
            else :
                movej(position_obj, vel=VELOCITY, acc=ACC)
                wait(0.5)
                # check_force_condition = 10
            time.sleep(0.5)  # 힘 조건을 확인하며 이동
            pass
    ...
    while rclpy.ok():
        ...
        modify_Ready_j = modify_position_value(Ready_j, 5, 45, 'posj')
        ...
        # 현재 posx 좌표의 2번째인덱스(높이) 조절
        Ready_x_2= posx(get_current_posx()[0])
        modify_Ready_x_2 = modify_position_value(Ready_x_2, 2, 0, 'posx')

        # 힘이 느껴질때까지 이동 후 닿았다면 posx 좌표 얻어오기
        force_based_movement(modify_Ready_x_2, 'posx', VELOCITY, ACC, max_force=5)
        ...
        # 2번째인덱스(높이) 가 10 이하라면 땅에 닿은것
            ...
            modify_go_ground = modify_position_value(go_ground, 2, 0, 'posx')

            # 힘이 느껴질때까지 이동 후 닿았다면 posx 좌표 얻어오기
            force_based_movement(modify_go_ground, 'posx', VELOCITY, ACC, max_force=5)
